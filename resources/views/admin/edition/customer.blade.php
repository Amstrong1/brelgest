                ...
                <form action="{{ route('admin.invoice.store') }}" method="post">
                    ...
                    <div class="lg:flex lg:justify-between lg:mr-32">
                        ...
                        <div>
                            <div class="relative w-64 max-w-xl mr-6 ml-6 mt-6 mb-6 focus-within:text-purple-500">
                                ...
                                <input name="num_fact" type="number" value="<?php if (isset($invoices->NumFacture)) {
                                    echo $invoices->NumFacture + 1;
                                } else {
                                    echo '1';
                                } ?>" disabled
                    ...
                    <div id="for_print">
                        <div id="entete" class="hidden p-4 text-xs mb-4">
                            <div class="mb-4">
                                @php
                                    $infos = DB::table('gmonentreprise')
                                        ->select('Denomination', 'NumIFU', 'RCCM')
                                        ->where('CodeStruct', '=', Auth::user()->CodeStruct)
                                        ->where('effacer', '=', 0)
                                        ->first();
                                @endphp
                                <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
                                    {{ $infos->Denomination }}</h2>
                                <span class="mb-2">IFU : {{ $infos->NumIFU }}</span> <br>
                                <span class="mb-2">RCCM : {{ $infos->RCCM }}</span>
                            </div>

                            <div class="bg-gray-100 p-4 flex justify-between items-stretch">
                                <div>
                                    <p class="mb-2">CLIENT : {{ $infos->NumIFU }}</p>
                                    <p class="mb-2">IFU : {{ $infos->NumIFU }}</p>
                                    <p class="mb-2">TEL : {{ $infos->RCCM }}</p>
                                </div>
                                <div>
                                    <p class="mb-2">Facture N° : @if (isset($invoices->NumFacture))
                                            {{ $invoices->NumFacture + 1 }}
                                        @else
                                            {{ 1 }}
                                        @endif
                                    </p>
                                    <p class="mb-2">Date d'émission : @php echo date('d-m-Y') @endphp</p>
                                    <p class="mb-2">Opérateur : {{ Auth::user()->Login }}</p>
                                </div>
                            </div>
                        </div>
